...
if($mainpage && !isset($_GET['feid']) && $_SESSION['tinybrowser']['integration'] == 'tinymce')
	{?>
...
elseif($mainpage && !isset($_GET['feid']) && $_SESSION['tinybrowser']['integration'] == 'fckeditor')
	{?>
	function selectURL(url){
...
elseif($mainpage && $_GET['feid'] != '')
	{?>
	function selectURL(url) {
	opener.document.getElementById("<?php echo filter_str($_GET['feid']); ?>").value = url;
	// Set img source of element id, if img id exists (format is elementid + "img")
	if (
        typeof(opener.document.getElementById("<?php echo filter_str($_GET['feid']); ?>img")) != "undefined"
        && opener.document.getElementById("<?php echo filter_str($_GET['feid']); ?>img") != null
        && opener.document.getElementById("<?php echo filter_str($_GET['feid']); ?>img").src.length != 0
		)
	   {
		opener.document.getElementById("<?php echo filter_str($_GET['feid']); ?>img").src = url;
		}
	self.close();
	}
	<?php
